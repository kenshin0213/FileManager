2021.11.03
1. Visual Studio 2013에서 작업 중
2. myDB 합치는 작업 중

2021.02.07
기존에 만들었던 DBManager 를 잘 사용하다가
일괄로 DB처리하는 기능 말고 개별로 파일(CSV)로 변환할 필요성이 생겨서
DBManager 폴더를 복사해서 FileManager를 하나 만들었음.
현재 FileManager에는 개별로 CSV로 변환하는 기능이 들어가 있지만, DBManager에는 없음.
그러니까 FileManager가 최신이니 FileManager로 대체하면 될듯함.

================================================================================
    MFC 라이브러리 : DBManager 프로젝트 개요
================================================================================

응용 프로그램 마법사에서 이 DBManager 응용 프로그램을 
만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 
프로그램작성을 위한 기본 구조를 제공합니다.

이 파일에는 DBManager 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
DBManager.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CDBManagerApp 응용 프로그램
    클래스를 선언합니다.

DBManager.cpp
    응용 프로그램 클래스 CDBManagerApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
res\DBManager.ico
    ...
    주 리소스 파일인 DBManager.rc에 의해 포함됩니다.
...
DBManagerDlg.h, DBManagerDlg.cpp - 대화 상자
    이 파일에는 CDBManagerDlg 클래스가 들어 있습니다. 이 클래스는
    응용 프로그램의 주 대화 상자 동작을 정의합니다. 대화 상자의 템플릿은
    Microsoft Visual C++에서 편집할 수 있는 DBManager.rc에 있습니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 DBManager.pch와
